(() => {
  console.log('Workverse Mail v2.4 loaded');
  ...
  function recipientList() {
    const emails = [...new Set(($("recipients").value || "").split(/[\s,;]+/).map(x => x.trim().toLowerCase()).filter(Boolean))];
  ...
  if (foreColor) foreColor.addEventListener("change", e => { document.execCommand("foreColor", false, e.target.value); const b = $("body"); if (b) b.focus(); });

  const backColor = $("backColor");
  if (backColor) backColor.addEventListener("change", e => { document.execCommand("hiliteColor", false, e.target.value); const b = $("body"); if (b) b.focus(); });

  const fontSize = $("fontSize");
  if (fontSize) fontSize.addEventListener("change", e => { document.execCommand("fontSize", false, e.target.value); const b = $("body"); if (b) b.focus(); });
  ...
  if (linkBtn && linkModal) {
    ...
    if (cancelLink) cancelLink.addEventListener("click", () => { linkModal.hidden = true; });
    const confirmLink = $("confirmLink");
    if (confirmLink) {
      confirmLink.addEventListener("click", () => {
        const url = ($("linkUrl").value || "").trim();
        const text = ($("linkText").value || "").trim();
        if (!url) { linkModal.hidden = true; return; }
        const html = text ? `<a href="${url.replace(/"/g, '&quot;')}">${text.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</a>` : `<a href="${url.replace(/"/g, '&quot;')}">${url.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</a>`;
  ...
  if (sendBtn) {
    sendBtn.addEventListener("click", async () => {
      ...
      if (!list.length || list.length > 100 || !subject || !bodyText.trim()) {
        if (statusEl) statusEl.textContent = "Enter 1-100 recipients, a subject and a body.";
        return;
      }
      ...
      try {
        const result = await api("/send", { method: "POST", body: JSON.stringify({ recipients: list, subject, body: html, attachments }) });
        if (statusEl) statusEl.textContent = `Done: ${result.successful} sent, ${result.failed?.length || 0} failed.`;
        await loadHistory();
      } catch (e) {
  ...
  async function loadHistory() {
    ...
    try {
      ...
      if (!items.length) { if (hist) hist.innerHTML = '<div class="empty">No emails sent yet.</div>'; return; }
      if (hist) {
        hist.innerHTML = items.map(x => `
          <div class="history-item">
            <div class="history-subject">${escapeHtml(x.subject || "")}<span class="badge">${escapeHtml(x.status || "")}</span></div>
